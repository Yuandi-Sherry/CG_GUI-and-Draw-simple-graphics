...
void Homework3::drawPoint(const float location[3], const float color[3]) {
	float pointVertex[6];
	for (int i = 0; i < 3; i++) {
...
void Homework3::drawLine(const float startPoint[2], const float endPoint[2]) {
...
void Homework3::drawCircle() {
	...
	while (x_i < center[0] + sqrt(2) * radius/2 ) {
		float loc[3] = { x_i, y_i, 0 };
		draw8points(x_i, y_i);
		if (d_i < 0) {
